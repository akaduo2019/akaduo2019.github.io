...
# 输入函数
def in_put():
    input_A = input("注:请用英文逗号,大括号,大于号和小于号,且A上的R集合中元素只限于序偶形式\n输入集合A,以回车结束\n")
    input_R = input("输入集合R,以回车结束\n")
    buffer = ''
    # 将输入1存放入A集合
    for i in range(1, len(input_A)):
        if input_A[i] != ',' and input_A[i] != '}':
            buffer = buffer + input_A[i]
        else:
            A.append(buffer)
            buffer = ''
    # 将输入2存放入R集合
    for j in range(1, len(input_R)):
        if input_R[j] == '<' or input_R[j] == '}':
            continue
        elif input_R[j] == ',':
            if input_R[j - 1] == '>':
                continue
